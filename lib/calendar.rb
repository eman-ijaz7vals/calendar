...
class Calendar
 ...
 # User can add/remove/edit an event to a specific date in a calendar.
  ...
  def editEvent(date, old_desc, new_desc)
    month , day, year = parseDate(date) ## return format - > June , 2, 1999
    even_arr = @cal[year][month][year + "-" + month + "-" + day]
    @cal[year][month][year + "-" + month + "-" + day].each_with_index { |a_event, index|
      if a_event.detail == old_desc 
        ...
        @cal[year][month][year + "-" + month + "-" + day][index].detail = new_desc
  ...
  def eventDetails(date)
     ...
     # return "No events to list for this date ! " if events.length == 0
     events.each_with_index { |event, index|

      puts "#{event.printDetails()}"

     }
     return "All events of given date are printed successfully!"
  end
  ...
  def allMonthEventsDetails(month, year)
    ...
    return "No events to list for this month ! " if month_events.length == 0
    month_events.each { |date, events_arr|
    ...
    return "All events of given month are printed successfully!"
  end
